...
Node.prototype.startWeb3Connection = function()
{
	...
	// Extend to custom txpool method
	web3._extend({
		property: 'txpool',
...
Node.prototype.getStats = function(forced)
{
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		function (err, results)
		{
			...
			if (results.peers) {
				CACHED_PEERS = results.peers;
			}

			if(results.peers !== null)
			{
				self.stats.active = true;
				self.stats.peers = results.peers ?? CACHED_PEERS;
				self.stats.mining = results.mining ?? false;
				self.stats.hashrate = results.hashrate;
				self.stats.gasPrice = results.gasPrice ? results.gasPrice.toString(10) : '0';

				if (results.syncing?.sync?.currentBlock) {
...
Node.prototype.getPending = function()
{
	...
	if (this._web3)
	{
		...
		web3.eth.getBlockTransactionCount('pending', function (err, pending)
		{
			...
			if (!pending) {
				pending = web3.txpool.stats()?.pending;
			}
